...
subplot(2,1,2)
maboxplot(log2(normCounts(:,1:4)),'title','G Normalized Read Count','orientation','horizontal')
ylabel('sample');
xlabel('log2(counts)');
...
for i=1:k
    ...
    LCModel = fitclinear(x_train,t_train,'Learner','logistic');
    num_theta = numel(LCModel.Beta);
    num_bias = numel(LCModel.Bias);

    % Validazione
    [Y_train_pred, score_train_pred] = LCModel.predict(x_train);
    [Y_test_pred, score_test_pred] = LCModel.predict(x_test);
    ...
    AUC_cv(i)=AUC;
    % calcparam train
    C_train = confusionmat(t_train,Y_train_pred);
    [accuracy(i,1),precision(i,1),recall(i,1),jaccard(i,1),fscore(i,1)]=calcparam(C_train);
    % calcparam test
    C_test = confusionmat(t_test,Y_test_pred);
